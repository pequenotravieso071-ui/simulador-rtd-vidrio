...
if 'fecha_txt' not in st.session_state:
    st.session_state.fecha_txt = datetime.date.today().strftime("%d/%m/%Y")
if 'hora_txt' not in st.session_state:
...
with st.sidebar.form("formulario_datos"):
    ...
    hora_input = st.text_input("Hora (Formato: HH:MM de 24h)", value=st.session_state.hora_txt)
...
# --- VALIDACIÓN DE LO QUE TECLEÓ EL OPERADOR ---
error_formato = False
try:
    # Intenta armar la fecha real con lo que se escribió
    inicio_real = datetime.datetime.strptime(f"{fecha_input.strip()} {hora_input.strip()}", "%d/%m/%Y %H:%M")
    
    # Si es correcto, bloquea los valores en la memoria para que no se pierdan jamás
    st.session_state.fecha_txt = fecha_input
    st.session_state.hora_txt = hora_input
except ValueError:
    error_formato = True
    st.error("⚠️ Formato incorrecto. Por favor escribe la fecha como DD/MM/AAAA (ej: 02/07/2026) y la hora como HH:MM en formato de 24 horas (ej: 14:30).")
    # Respaldo de seguridad para que no colapse el resto de la app gráfica
    inicio_real = datetime.datetime.now()

# Solo si el texto ingresado es válido desplegamos el análisis
